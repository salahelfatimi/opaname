...
export default function PizzeriaHome(){
    ...
    return(
      ...
      <video className="w-full h-full object-cover" title="French O'Paname" autoPlay  loop playsInline muted>
        <source src="pizzeria/bgVideo.mp4" type="video/mp4" />
        Your browser does not support the video tag.
      </video>
    
      <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-6">
        <Link href="/pizzeria/menu-pizzeria" className="bg-primary text-white text-xl font-black py-3 px-8 rounded-full border-4 border-primary hover:bg-secondary hover:border-white duration-700">
        ...
        <div className="relative">
          ...
          <div className={`absolute top-16 left-0 right-0 bg-primary text-white font-bold rounded-lg p-4 flex flex-col gap-3 items-center transition-opacity duration-300 ${open ? 'opacity-100' : 'opacity-0 pointer-events-none'} w-80`}>
            <Link href="/pizzeria/menu-pizzeria" className="w-full text-center bg-primary hover:bg-secondary py-3 px-4 rounded-full duration-500">
